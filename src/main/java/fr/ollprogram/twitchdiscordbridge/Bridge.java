...
/**
 * A bridge represent a link between Twitch and Discord. It allows sending messages on both sides.
 *
 * @author ollprogram
 */
public final class Bridge {
  ...
  public void sendToTwitch(Message msg) {
    ...
    if (cleanMessage.contains("https://tenor.com/view/")
        || FilteringUtils.isEmptyString(cleanMessage)) return;
    // ignoring tenor gifs messages and messages without text content (files)
    botChat.sendMessage(
        twitchChannelName, "[DISCORD] " + msg.getAuthor().getName() + " says : " + cleanMessage);
  }
  ...
  public void sendToDiscord(ChannelMessageEvent msg) {
    ...
    try {
      discordChannel
          .sendMessage("[TWITCH] " + msg.getUser().getName() + " says : " + filteredMessage)
          .queue();
    } catch (NullPointerException e) { // if the discord channel isn't defined in settings.
      System.out.println(
          "Something is wrong with the discord channel ID."
