...
public class Main {
    ...
    try {
      ...
      TwitchClient twitchClient =
          TwitchClientBuilder.builder()
              .withEnableChat(true)
              .withChatAccount(settings.getTwitchAccount())
              .build();
      ...
      jda.getPresence()
          .setActivity(Activity.listening("Twitch : " + settings.getTwitchChannelName()));
      Bridge bridge = new Bridge(settings, twitchClient.getChat(), jda);

      TextCommand[] commands = {
        new Test("test", Permission.ADMINISTRATOR), // define all discord commands
        new Prefix("prefix", Permission.ADMINISTRATOR),
        new SrcCode("code"),
        new BridgeCmd("bridge", Permission.ADMINISTRATOR, bridge)
      };
      ...
      twitchClient
          ...
          .registerListener(new TwitchListener(bridge));
      ...
      ConsoleCommand[] consoleCommands = {
        new Shutdown("shutdown", jda, twitchClient),
        new Say("say", bridge),
  ...
  private static void consoleInterface(ConsoleCommand[] commands, @NotNull JDA jda) {
    ...
    System.out.println(
        "[INFO] Your bots must be private, and the bots are recommended to be used only on one"
            + " server.\n");
    System.out.println(
        "[INFO] It's also recommended to disable links for everyone on the discord channel targeted"
            + " and the twitch channel targeted.\n");
    System.out.println(
