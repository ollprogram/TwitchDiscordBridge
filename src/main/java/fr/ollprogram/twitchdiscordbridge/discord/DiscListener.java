...
public class DiscListener extends ListenerAdapter {
	...
	/**
	 * Handle every message event. Allows the recognition of a use of a commands in the chat and execute them if possible.
	...
	public void onMessageReceived(@NotNull MessageReceivedEvent event) {
		...
		if(cmd.havePermission(event.getMember())){
			...
			event.getAuthor().openPrivateChannel().queue(channel ->
					channel.sendMessage(event.getAuthor().getAsMention()+" You don't have the permission ["+
					cmd.getPermName()+"] to use this command.").queue()
			);
		}
	}
	...
	private static void helpAction(TextCommand[] commands, MessageReceivedEvent event){
		...
		for(TextCommand cmd : commands){
			msg.append("- ").append(cmd.getName()).append(" : ").append(cmd.getDescription()).append("\n\n");
		}
		msg.append(" ```");
		event.getAuthor().openPrivateChannel().queue(channel ->
