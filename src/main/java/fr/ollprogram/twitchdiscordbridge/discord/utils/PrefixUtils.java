...
public abstract class PrefixUtils {
  ...
  public static String retrievePrefix(Guild guild) {
    ...
    if (botNickname.charAt(0) == '<') {
      for (int i = 1; i < botNickname.length() - 1 && botNickname.charAt(i) != '>'; i++) {
        ...
        if (i == botNickname.length() - 2 && botNickname.charAt(botNickname.length() - 1) != '>')
          return "!";
      }
   ...
   *
   * @param guild The guild
   * @param newPrefix The new prefix to set. <code>"reset"</code> to reset the prefix and the bot
   *     nickname.
   */
