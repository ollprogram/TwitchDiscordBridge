...
public class BridgeCmd extends TextCommand {
  ...
  /**
   * Can change the discord channel targeted. Can open or close the bridge or get information about
   * it.
   *
  ...
  protected void action(@NotNull String @NotNull [] args, @NotNull MessageReceivedEvent event) {
    ...
    if (args.length >= 2) {
      if (args[1].equalsIgnoreCase("target")) {
        ...
        channel
            .sendMessage(
                event.getAuthor().getAsMention() + " Target channel updated: " + channel.getName())
            .queue();
        return;
      }
        ...
        channel
            .sendMessage(
                event.getAuthor().getAsMention()
                    + " Here are the information about the bridge: \n"
                    + "```scala\nDiscordTargetChannelName : "
                    + bridge.getDiscordChannel().getName()
                    + "\n"
                    + "TwitchTargetChannelName : "
                    + bridge.getTwitchChannelName()
                    + "\n"
                    + "Status : "
                    + ((bridge.isOpened()) ? "opened" : "closed")
                    + "```")
            .queue();
        return;
      }
  ...
  public String getDescription() {
    return "Configs or shows information about the bridge. Parameters : \n"
               ...
               + "	- target : Changes the discord targeted channel to the channel where the command"
               + " have been called.\n"
