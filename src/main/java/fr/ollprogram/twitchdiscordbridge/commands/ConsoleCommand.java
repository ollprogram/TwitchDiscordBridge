...
public abstract class ConsoleCommand {
  ...
  public void execute(@NotNull String consoleLine) {
    ...
    if (args[0].length() != name.length())
      return; // avoid for example that !testHello stills working as a test command
    action(args);
  }
   ...
   * @return The retrieved command in the array with the specified command line. null if no command
   *     found.
   */
  public static @Nullable ConsoleCommand retrieve(
      ConsoleCommand @NotNull [] commands, String cmdLine) {
