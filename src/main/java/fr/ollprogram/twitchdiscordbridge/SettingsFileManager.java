...
/**
 * Represent all settings present in the settings file. These settings are important for the
 * application to save information for the next relaunch. Settings are always updated in files when
 * modified.
 *
 * @author ollprogram
 */
public class SettingsFileManager {
  ...
  private void loadFromFile() throws IOException, SettingsFileSyntaxException {
    ...
    reader.close();
    fillFields(lines);
    if (isUndefined(discordToken)
        || isUndefined(discordChannelID)
        || isUndefined(twitchChannelName)
        || twitchAccount == null) {
      throw new SettingsFileSyntaxException(
          "WRONG settings contents : \n" + file.getAbsolutePath());
    }
  }
